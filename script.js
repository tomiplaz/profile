...
const topElemRect = document.getElementsByClassName('container--first').item(0).getBoundingClientRect();
...
if (isMobile) {
    ...
    window.addEventListener('scroll', () => {
        if (topElemRect.top === 0) mobileTopScroll();
    });
}

function mobileTopScroll() {
    setTimeout(() => {
        window.scrollTo(0, 1);
    }, 10);
}
