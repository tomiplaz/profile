...
const containerFirstElem = document
  .getElementsByClassName("container--first")
  .item(0);
...
(function () {
  ...
  window.addEventListener("scroll", () => {
    ...
    indicatorElem.className =
      containerFirstElemRect.top < -20 ? "hide" : "show";
  });
})();
...
(function () {
  ...
  const HEARTBEAT_COUNTDOWN = "heartbeat-countdown";
  ...
  const COMMAND_URLS = {
    ...
    [BLOGS]: "https://blogs.tomiplaz.dev",
    [YAMB]: "https://yamb.tomiplaz.dev",
    [YAMB_V2]: "https://yamb.tomiplaz.dev",
    [YAMB_V1]: "https://ugodnomjesto.000webhostapp.com/yamb/",
    [HEARTBEAT_COUNTDOWN]: "https://tomiplaz.dev/heartbeat-countdown",
  };

  const COMMAND_NOT_FOUND_MESSAGE = `Invalid command!\nRun '${HELP}' to get a list of valid commands.`;
  const HELP_MESSAGE =
    "Valid CLI commands for tomiplaz.dev:\n\n" +
    [
      ...
      `${HEARTBEAT_COUNTDOWN}: ${OPEN} Love Timer demonstration`,
    ].join("\n");
    ...
    switch (event.key) {
      ...
      default:
        if (isCtrlDown && event.key === C_KEY) {
          ...
          !(
            inputElem.textContent.substr(-1) === SPACE_KEY &&
            event.key === SPACE_KEY
          )
        ) {
